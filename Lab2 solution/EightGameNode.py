...
class EightGameNode:
    ...
    def copy(self):
        ...
        new_node = EightGameNode(new_board, self.cost)
        return new_node
    ...
    def manhattan_distance(self, goal):
        ...
        for i in range(3):
            for j in range(3):
                for k in range(3):
                    for l in range(3):
                        if goal.board[i][j] == self.board[k][l] and goal.board[i][j] != 0:
                            move_sum += abs(i-k) + abs(j-l)
        return move_sum

    def own_heuristic(self, goal):
        if self.__eq__(goal):
            return 0
        move_sum = 0
        for i in range(3):
            for j in range(3):
                if goal.board[i][j] != self.board[i][j]:
                    move_sum += 1.7777777
        return int(move_sum)
